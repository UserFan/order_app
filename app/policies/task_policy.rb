class TaskPolicy < ApplicationPolicy
  ...
  def closing?
    user.moderator? || user.super_admin? || user.guide? ||
    record.user == user || !record.executions.present?

  end

  def show_detail?
    user.moderator? || user.super_admin? || user.guide? || record.user == user
  end

  def permitted_attributes
    [:type_document_id, :date_open, :date_execution, :structural_id,
     :description, :date_closed, :user_id, :status_id, :task_number,
     images_document: []]
  end
